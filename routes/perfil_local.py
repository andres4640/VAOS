...
@routes.route("/eliminar_local", methods=["POST"])
def eliminar_local():
    try:
        if "iduser" in session:
            if session["esEmp"] == 1:
                localId = request.args.get('localid')
                local = db.session.query(Local).filter(Local.id == localId).first()
                for even in local.eventos:
